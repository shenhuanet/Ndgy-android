...
public class HighlightView {
    ...
    public int getHit(float x, float y) {
        ...
        boolean horizonCheck = (x >= r.left - hysteresis)
        ...
        if ((Math.abs(r.top - y) < hysteresis) && horizonCheck) {
            result |= GROW_TOP_EDGE;
        }
        if ((Math.abs(r.bottom - y) < hysteresis) && horizonCheck) {
            result |= GROW_BOTTOM_EDGE;
        }
        if (result == GROW_NONE && r.contains((int) x, (int) y)) {
            result = MOVE;
        }
        return result;
    }
    ...
    void moveBy(float dx, float dy) {
        Rect invalidRect = new Rect(drawRect);
        ...
        drawRect = computeLayout();
        invalidRect.union(drawRect);
        invalidRect.inset(-(int) handleRadius, -(int) handleRadius);
        viewContext.invalidate(invalidRect);
    }
