...
import com.shenhua.lib.boxing.loader.IBoxingCallback;
...
import com.shenhua.lib.boxing.presenter.PickerContract;
...
import static com.shenhua.lib.boxing.utils.Contants.EXTRA_ALBUM_ID;
...
import static com.shenhua.lib.boxing.utils.Contants.EXTRA_START_POS;
...
public abstract class AbsBoxingViewActivity extends AppCompatActivity implements PickerContract.View {
    ...
    private void parseSelectedMedias(Bundle savedInstanceState, Intent intent) {
        if (savedInstanceState != null) {
            mSelectedImages = savedInstanceState.getParcelableArrayList(EXTRA_SELECTED_MEDIA);
            mAlbumId = savedInstanceState.getString(EXTRA_ALBUM_ID);
            mStartPos = savedInstanceState.getInt(EXTRA_START_POS, 0);
        } else if (intent != null) {
            mStartPos = intent.getIntExtra(EXTRA_START_POS, 0);
            mSelectedImages = intent.getParcelableArrayListExtra(EXTRA_SELECTED_MEDIA);
    ...
    public void onSaveInstanceState(Bundle outState, PersistableBundle outPersistentState) {
        ...
        outState.putParcelable(EXTRA_CONFIG, BoxingManager.getInstance().getBoxingConfig());
    }
    ...
    public final int getMaxCount() {
        BoxingConfig config = BoxingManager.getInstance().getBoxingConfig();
        if (config == null) {
            return Contants.DEFAULT_SELECTED_COUNT;
        }
